...
from utils.rows import compare_rows
...
def get_new_session(event_url):
    ...
    if not logs:
        raise RuntimeError("Failed to capture Ticketmaster session headers")
...
def fetch_prices(event_id, sections, max_price, max_row, tickets, offset=0):
    ...
    params = {
        ...
        'apisecret': os.environ.get('TM_API_SECRET'),
...
def fetch_all_prices(event_id, sections, max_price, max_row, tickets):
    ...
    while True:
        print(f"Fetching prices with offset {offset}...")
